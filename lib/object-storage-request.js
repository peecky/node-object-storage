...
ObjectStorageRequest.prototype.end = function () {
	...
	if (this.attachment) {
		var readStream = (typeof this.attachment === 'object' && this.attachment.readable)
			?  readStream = this.attachment	// read stream itself
			: readStream = fs.createReadStream(this.attachment);
		readStream.pipe(request(opts, handleResponse));
		if (typeof readStream.resume === 'function') {
			try {
				// try to call resume() for a paused stream
				readStream.resume();
			}
			catch(e) {}
		}
